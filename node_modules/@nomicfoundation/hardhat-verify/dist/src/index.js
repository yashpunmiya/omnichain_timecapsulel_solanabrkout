...
(0, config_1.subtask)(task_names_2.TASK_VERIFY_RESOLVE_ARGUMENTS)
    ...
    .addFlag("noCompile")
    .setAction(async ({ address, constructorArgsParams, constructorArgs: constructorArgsModule, contract, libraries: librariesModule, listNetworks, noCompile, }) => {
...
(0, config_1.subtask)(task_names_2.TASK_VERIFY_ETHERSCAN)
    ...
    .addFlag("listNetworks")
    .addFlag("noCompile")
    .setAction(async ({ address, constructorArgs, libraries, contractFQN, listNetworks, noCompile, }, { config, network, run }) => {
    if (listNetworks) {
...
(0, config_1.subtask)(task_names_2.TASK_VERIFY_ETHERSCAN_GET_CONTRACT_INFORMATION)
    ...
    if (contractFQN !== undefined) {
        ...
        if (contractInformation === null) {
            throw new errors_1.DeployedBytecodeDoesNotMatchFQNError(contractFQN, network.name);
        }
    }
    else {
...
(0, config_1.subtask)(task_names_2.TASK_VERIFY_VERIFY)
    ...
    .addOptionalParam("contract")
    .addFlag("noCompile")
    .setAction(async ({ address, constructorArguments, libraries, contract, noCompile, }, { run }) => {
    if (address === undefined) {
        throw new errors_1.MissingAddressError();
    }
    const { isAddress } = await Promise.resolve().then(() => __importStar(require("@ethersproject/address")));
